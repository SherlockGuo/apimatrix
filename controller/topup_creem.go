...
func verifyCreemSignature(payload string, signature string, secret string) bool {
	if secret == "" {
		logger.LogWarn(context.Background(), fmt.Sprintf("Creem webhook secret 未配置 test_mode=%t signature=%q %s", setting.CreemTestMode, signature, common.RedactedBodyLog("body", len(payload))))
		if setting.CreemTestMode {
			logger.LogInfo(context.Background(), fmt.Sprintf("Creem webhook 验签已跳过 reason=test_mode signature=%q %s", signature, common.RedactedBodyLog("body", len(payload))))
			return true
		}
		return false
	}
...
func RequestCreemPay(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem 支付请求已收到 user_id=%d %s", c.GetInt("id"), common.RedactedBodyLog("body", len(bodyBytes))))
...
func CreemWebhook(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem webhook 收到请求 path=%q client_ip=%s signature=%q %s", c.Request.RequestURI, c.ClientIP(), signature, common.RedactedBodyLog("body", len(bodyBytes))))
	if signature == "" {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Creem webhook 缺少签名 path=%q client_ip=%s %s", c.Request.RequestURI, c.ClientIP(), common.RedactedBodyLog("body", len(bodyBytes))))
	...
	if !verifyCreemSignature(string(bodyBytes), signature, setting.CreemWebhookSecret) {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Creem webhook 验签失败 path=%q client_ip=%s signature=%q %s", c.Request.RequestURI, c.ClientIP(), signature, common.RedactedBodyLog("body", len(bodyBytes))))
		c.AbortWithStatus(http.StatusUnauthorized)
		return
	}
	...
	if err := c.ShouldBindJSON(&webhookEvent); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem webhook 解析失败 path=%q client_ip=%s error=%q %s", c.Request.RequestURI, c.ClientIP(), err.Error(), common.RedactedBodyLog("body", len(bodyBytes))))
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}
...
func genCreemLink(ctx context.Context, referenceId string, product *CreemProduct, email string, username string) (string, error) {
	...
	logger.LogInfo(ctx, fmt.Sprintf("Creem API 响应已收到 trade_no=%s status_code=%d %s", referenceId, resp.StatusCode, common.RedactedBodyLog("response body", len(body))))
