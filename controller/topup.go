...
func RequestEpay(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 充值订单创建成功 user_id=%d trade_no=%s payment_method=%s amount=%d money=%.2f uri=%q params_redacted=true", id, tradeNo, req.PaymentMethod, req.Amount, payMoney, uri))
...
func EpayNotify(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 webhook 收到请求 path=%q client_ip=%s method=%s params_redacted=true count=%d", c.Request.RequestURI, c.ClientIP(), c.Request.Method, len(params)))
	...
	if err == nil && verifyInfo.VerifyStatus {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 webhook 验签成功 trade_no=%s callback_type=%s trade_status=%s client_ip=%s verify_info_redacted=true", verifyInfo.ServiceTradeNo, verifyInfo.Type, verifyInfo.TradeStatus, c.ClientIP()))
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp == nil {
			logger.LogWarn(c.Request.Context(), fmt.Sprintf("易支付 回调订单不存在 trade_no=%s callback_type=%s client_ip=%s verify_info_redacted=true", verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP()))
			return
		}
		...
		if topUp.Status == common.TopUpStatusPending {
			...
			err := topUp.Update()
			if err != nil {
				logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 更新充值订单失败 trade_no=%s user_id=%d client_ip=%s error=%q topup_redacted=true", topUp.TradeNo, topUp.UserId, c.ClientIP(), err.Error()))
				return
			}
			...
			if err != nil {
				logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 更新用户额度失败 trade_no=%s user_id=%d client_ip=%s quota_to_add=%d error=%q topup_redacted=true", topUp.TradeNo, topUp.UserId, c.ClientIP(), quotaToAdd, err.Error()))
				return
			}
			logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 充值成功 trade_no=%s user_id=%d client_ip=%s quota_to_add=%d money=%.2f topup_redacted=true", topUp.TradeNo, topUp.UserId, c.ClientIP(), quotaToAdd, topUp.Money))
			model.RecordTopupLog(topUp.UserId, fmt.Sprintf("使用在线充值成功，充值金额: %v，支付金额：%f", logger.LogQuota(quotaToAdd), topUp.Money), c.ClientIP(), topUp.PaymentMethod, "epay")
		}
	} else {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 webhook 忽略事件 trade_no=%s callback_type=%s trade_status=%s client_ip=%s verify_info_redacted=true", verifyInfo.ServiceTradeNo, verifyInfo.Type, verifyInfo.TradeStatus, c.ClientIP()))
	}
}
